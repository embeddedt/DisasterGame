...
import { GreenSecurityIcon, BlueHouseIcon, PurpleInfoIcon, FontAwesomeIcon, RedBombIcon, TerraformUpIcon, TerraformDownIcon, WaterIcon, LandIcon, UnhousedPersonIcon } from './Icons';
...
const TileOverlay = React.forwardRef<HTMLDivElement, TileOverlayProps>((props, ref) => {
    ...
    return  <div className={`tile-overlay-container tile-overlay-${props.isSelected ? "selected" : "unselected"} ${props.embed ? "tile-overlay-embed" : ""}`} ref={ref} style={props.style}>
        ...
        <div className="tile-info-container">
            {props.name}
            <p></p>
            <span className="icon-information"><span><FontAwesomeIcon type="exclamation-triangle" color="#fccf03"/>&nbsp;{"100%"}</span>&nbsp;<span><UnhousedPersonIcon/>&nbsp;{props.unhoused}</span>{residencyComponent}</span>
        </div>
    </div>;
    /*</div>
        </div>
